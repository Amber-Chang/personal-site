...
- 網站已具備：
  ...
  - `/projects/[slug]`
- 內容來源目前是：
  - blog：預設走 Supabase repository；若 preview / build 缺少 public Supabase env，公開讀取會退回 repo 內 Markdown fallback
  - projects：預設走 Supabase repository；若 preview / build 缺少 public Supabase env，公開讀取會退回 repo 內 Markdown fallback
- repo 內的 `content/posts/*.md`、`content/projects/*.md` 仍主要作為 migration / import source；但現在也作為缺少 public env 時的公開 fallback source
